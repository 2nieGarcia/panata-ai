...
INSERT INTO buckets (name, description) VALUES
    ('School', 'University coursework, assignments, and academic tasks'),
    ('Personal', 'Personal life, errands, self-care, and daily tasks'),
    ('Side Projects', 'Hackathons, portfolio projects, learning, and experiments'),
    ('Health', 'Fitness, medical appointments, mental health, and wellness'),
    ('Work', 'Freelance work, internships, and professional tasks'),
    ('Finance', 'Budget tracking, bills, investments, and financial tasks');

-- Create "General" project in each bucket
INSERT INTO projects (bucket_id, name, description)
SELECT id, 'General', 'Default project for quick tasks'
FROM buckets;
...
-- Get project by name
CREATE OR REPLACE FUNCTION get_project_by_name(p_name TEXT)
RETURNS TABLE (id UUID, name TEXT, bucket_id UUID, bucket_name TEXT) AS $$
BEGIN
    RETURN QUERY
    SELECT p.id, p.name, p.bucket_id, b.name
    FROM projects p
...
CREATE OR REPLACE FUNCTION get_project_in_bucket(p_name TEXT, b_name TEXT)
RETURNS TABLE (id UUID, name TEXT, bucket_id UUID, bucket_name TEXT) AS $$
BEGIN
    RETURN QUERY
    SELECT p.id, p.name, p.bucket_id, b.name
...
-- Get bucket by name
CREATE OR REPLACE FUNCTION get_bucket_by_name(b_name TEXT)
...
CREATE OR REPLACE FUNCTION get_general_project(b_name TEXT)
RETURNS TABLE (id UUID, name TEXT, bucket_id UUID, bucket_name TEXT) AS $$
BEGIN
    RETURN QUERY
    SELECT p.id, p.name, p.bucket_id, b.name
...
RETURNS TABLE (id UUID, name TEXT, bucket_id UUID, bucket_name TEXT, similarity REAL) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        p.id, p.name, p.bucket_id, b.name,
...
CREATE OR REPLACE FUNCTION get_upcoming_tasks(hours_ahead INTEGER DEFAULT 72)
RETURNS TABLE (
    task_id UUID, task_name TEXT, deadline TIMESTAMPTZ,
    priority INTEGER, project_name TEXT, bucket_name TEXT
) AS $$
BEGIN
    RETURN QUERY
    SELECT t.id, t.task_name, t.deadline, t.priority, p.name, b.name
    FROM tasks t
    JOIN projects p ON p.id = t.project_id
    JOIN buckets b ON b.id = p.bucket_id
    WHERE t.status = 'pending'
    AND (t.deadline IS NULL OR t.deadline <= NOW() + (hours_ahead || ' hours')::INTERVAL)
...
DECLARE
    new_task_id UUID;
BEGIN
    INSERT INTO tasks (project_id, task_name, description, deadline, priority)
    VALUES (p_project_id, p_task_name, p_description, p_deadline, p_priority)
    RETURNING id INTO new_task_id;
    
    INSERT INTO task_logs (task_id, action, summary)
    VALUES (new_task_id, 'created', 'Task created: ' || p_task_name);
...
BEGIN
    ...
    INSERT INTO task_logs (task_id, action, summary)
    VALUES (p_task_id, 'completed', 'Task marked as completed');
    RETURN TRUE;
END;
$$ LANGUAGE plpgsql;
